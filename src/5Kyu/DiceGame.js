...
function DiceEquivalent(array) {
  if (!(array.length >= 2 && array.length <= 7)) {
    return "There must be atleast 2 dices!";
  }
  let factors = [];
  let result = [];
  let target = array.reduce((final, val) => final * val);
  ...
  for (let i = 0; i < factors.length; i++) {
    for (let j = 0; j < i; j++) {
      if (factors[i] * factors[j] === target && i !== j) {
        result.push([factors[i], factors[j]]);
      } else if (factors.includes(factors[i] * factors[j])) {
        let val = factors[i] * factors[j];
        for (let k = 0; k < factors.length; k++) {
          if (val * factors[k] === target && i !== j) {
            result.push([factors[i], factors[j], factors[k]]);
          }
        }
      }
    }
